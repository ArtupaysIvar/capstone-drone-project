...
private:
    ...
    rclcpp::Time last_triangulation_time;
    double triangulation_timeout_ = 2.0; // seconds
...
DataCollectionNode::DataCollectionNode() : Node("data_collection_node"),
    ...
    last_triangulation_time(rclcpp::Time(0, 0, RCL_ROS_TIME))
    ...
    if (!csv_file_.is_open()) {
        ...
        csv_file_ << "mission_time_s,tx,ty,tz,error,d1,d2,d3,x1,x2,x3,y1,y2,y3,z1,z2,z3\n";
    ...
    act_target_location << 2.0f, 10.0f, 0.0f; 
...
void DataCollectionNode::csvLogging(){
    bool triangulation_fresh =
        (this->now() - last_triangulation_time).seconds() < triangulation_timeout_;
    
    int detection_count = pixel1_received + pixel2_received + pixel3_received;
    bool valid_triangulation = triangulation_fresh && (detection_count >= 2) && target_location_received;
